...
//	
//	Procedure file to work with Tektronics TDS300 series oscilloscopes, though built  
//	mostly for the 4-channel TDS3034.  Saves scope traces as waves Tek_Ch1, etc.
//	
//	Dependencies:  
...
//	
//	Notes:
// -- Shares global variables and some function names with the other Tek driver files Driver_TDS1000 and Driver_TDS2004B_VISA.
...
//	####[ Table of Contents ]####
//	Graphs:							Description:
//		displayTek						Makes a stylish graph of Tek_Ch1-4 waves (volts scale).  (From Brian Patton)
//	Functions:						Description:
//		saveTek							Saves captured scope traces as new waves with a waveid, to work with Expt_Analysis
//		getChan_TDS3K					Saves a trace from the o-scope into a global variable
//		getTekCh1 to 4					Shortcut to get Ch1 to 4, assumes devname = "scope"
//		getTekAll						Shortcut to get all channels, assumes devname = "scope"
//		setupAvg_TDS3K					Sets up scope in average mode
//		setupSample_TDS3K				Sets up scope in sample mode
...
Function getTekCh1()
	getChan_TDS3K("scope", 1)
End
Function getTekCh2()
	getChan_TDS3K("scope", 2)
End
Function getTekCh3()
	getChan_TDS3K("scope", 3)
End
Function getTekCh4()
	getChan_TDS3K("scope", 4)
End
Function getTekAll()
	getChan_TDS3K("scope", 1)
	getChan_TDS3K("scope", 2)
...
//setupAvg_TDS3K:  Sets up scope in average mode
Function setupAvg_TDS3K(devname, avg)
...
//setupSample_TDS3K:  Sets up scope in sample mode
Function setupSample_TDS3K(devname)
	string devname				//device name for GPIB calls
...
//getChan_TDS3K:  Saves a trace from the o-scope into a global variable
Function getChan_TDS3K(devname, chan)
	string devname				//device name for GPIB calls
	variable chan				//Get channel chan
	
	//Make sure you chose a valid channel!
	if((chan != 1) && (chan != 2) && (chan != 3) && (chan != 4))
		Print "#### ERROR during getChan_TDS3K:  Invalid channel selection = " + num2istr(chan) + "!"
		abort
	endif
	
	//Check to see if the trace is displayed, to avoid GPIB timeout error:
	if(!str2num(gpibQuery(devname, "SELECT:CH" + num2istr(chan) + "?")))
		Print "#### ERROR during getChan_TDS3K:  Channel " + num2istr(chan) + " not displayed!"
		abort
	endif
	
	//Put message on scope saying we're transferring data:
	gpibSend(devname,"MESSAGE:SHOW \"Transferring data over GPIB...\"")
	gpibSend(devname,"MESSAGE:STATE ON")
	
	//OPTIONAL:  Stop acquisition for transfer (save state to restore later)
	//NOTE:  This is only needed if you want to read the full PREAMBLE, for some reason
	//variable acq_flag = str2num(gpibQuery(devname,"ACQUIRE:STATE?"))	//1 = running, 0 = stopped
	//gpibSend(devname, "ACQUIRE:STATE OFF")
	...
	variable HorizScale = str2num(gpibQuery(devname,"HOR:MAIN:SCALE?"))		//Get horizontal time base (time/division)
	//variable Vscale = str2num(gpibQuery(devname,"CH" + num2istr(chan) + ":VOL?"))	// Read the voltage scale (volts)
	//variable Vpos = str2num(gpibQuery(devname,"CH" + num2istr(chan) + ":POS?"))		// Read the vertical position (divs)
	//variable Voffset = str2num(gpibQuery(devname,"CH" + num2istr(chan) + ":OFFS?"))	// Read the vertical offset (volts)
	...
	// Get non-data information from pre-amble
	//string preamble = gpibQuery(devname, "WFMPRE?")		//Read the full preamble (NEEDS acquisition stopped!).  Don't use this except for testing...
	...
	GPIBRead2/N=(2+ceil(log(RecLength+0.5))) dummy	// Read the preamble (to ignore it) of length 5 for 500, 7 for 10000 points
	GPIBReadBinaryWave2/TYPE=(0x10) data		// Ask for data stream, no "B" flag b/c RIBINARY indicates most-significant-byte first.  0x10 = 2 bytes/point.
	GPIBRead2 dummy									// Read post-amble, just to get rid of it.
	
	//Convert data to volts using non-preamble data: (Legacy, old Happer code method -- causes ERRORS!  See NOTE at start of document.)
	//data /= 6553.5						// Convert to div's. NOTE: RIBINARY & 2 bytes: 256^2 = 65536 values --> range of 65535.  10 divs/screen.
	//data -= Vpos							// Remove vertical position in divs
	//data *= Vscale						// Scale to voltage
	//data += Voffset						// Remove offset voltage
	...
	//OPTIONAL:  Restore previous acquire setting:  (ONLY needed if read whole preamble)
	//if(acq_flag == 1)	//If not stopped before, then start acquisition again.  (If stopped before, do nothing.)
	//	gpibSend(devname, "ACQUIRE:STATE ON")
	//endif
	
	//Remove message off scope screen
	gpibSend(devname,"MESSAGE:STATE OFF")
...
//####################################################################
// displayTDS3K and related functions

//Adds displayTDS3K to the Macros menu
Menu "Macros"
	"Display Tektronix Traces", displayTek()
End

//displayTek:  Makes a stylish graph of Tek_Ch1-4 waves (volts scale)
// (Modified from Brian Patton's code)  
Window displayTek() : Graph
	PauseUpdate; Silent 1		// building window...
	Display /W=(640.08,374.24,1060.56,641.36) Tek_Ch1,Tek_Ch2,Tek_Ch3,Tek_Ch4//,cnmr__FID_0
	//AppendToGraph cnmr__FID_1
	ModifyGraph wbRGB=(56576,56576,56576)
	ModifyGraph rgb(Tek_Ch1)=(52224,52224,0),rgb(Tek_Ch2)=(16384,48896,65280),rgb(Tek_Ch3)=(65280,0,52224)
	ModifyGraph rgb(Tek_Ch4)=(0,65280,0)//,rgb(cnmr__FID_1)=(0,39168,0)
	ModifyGraph grid=2, gridRGB=(48896,52992,65280), mirror=2, minor=1, standoff=0
	Label left "\\u#2Voltage (\\K(65280,65280,0)Ch1\\K(0,0,0), \\K(16384,48896,65280)Ch2\\K(0,0,0), \\K(65280,0,52224)Ch3\\K(0,0,0), \\K(0,65280,0)Ch4\\K(0,0,0))"
